...
const RoomPage: React.FC = () => {
  ...
  const shootingRef = useRef(false); // Prevent duplicate shots
  ...
  useEffect(() => {
    ...
    console.log(
      `🎯 Lines check: current=${currentLines}, previous=${previousLines}, shooting=${shootingRef.current}`
    );
    console.log(
      `🎯 Refs available: multiBoard=${!!multiBoardRef.current}, fireball=${!!fireballEffectRef.current}`
    );

    if (currentLines > previousLines && !shootingRef.current) {
      const linesCleared = currentLines - previousLines;
      console.log(`🔥 Lines cleared: ${linesCleared}, shooting fireball!`);

      shootingRef.current = true; // Prevent duplicate shots
      ...
      if (multiBoardRef.current && fireballEffectRef.current) {
        ...
        console.log(`🎯 Player positions found:`, positions);

        if (positions.length > 0) {
          // Only shoot one fireball to the first player
          const firstTarget = positions[0];
          console.log(`🔥 Shooting fireball to:`, firstTarget);
          fireballEffectRef.current.shootFireball(firstTarget.x, firstTarget.y);
        } else {
          console.warn(`🚫 No player positions found for fireball`);
        }
      } else {
        console.warn(
          `🚫 Missing refs - multiBoard: ${!!multiBoardRef.current}, fireball: ${!!fireballEffectRef.current}`
        );
      }
